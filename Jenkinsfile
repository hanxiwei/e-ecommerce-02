pipeline {
  ...
  docker-compose -p ${env.COMPOSE_PROJECT} exec -T api python -m pytest tests/ ${markerExpr} --cov=app --cov-report=html --cov-fail-under=80 --alluredir=/app/allure-results
else
  docker compose -p ${env.COMPOSE_PROJECT} exec -T api python -m pytest tests/ ${markerExpr} --cov=app --cov-report=html --cov-fail-under=80 --alluredir=/app/allure-results
fi""")
